...
def main():
    ...
    rows, cols = int(input_file.split('_')[1]),int(input_file.split('_')[2])
    print(rows,cols) #400,600
    ...
    for block_index in range(blocks_count):
        i = int(block_index // (blocks_count*8/rows) * block_side)
        j = int(block_index % (blocks_count*8/rows) * block_side)
        for c in range(3):
    ...
    image.save("new_"+str(input_file.split('_')[0])+".jpg")
    # image.show()
