...
def main():
    ...
    # h=np.array(image).shape[1]
    # w=np.array(image).shape[0]
    # h_size=h
    # w_size=w
    # if w%8!=0 :
    #     w_size=int(w/8)*8
    ...
    #     h_size=int(h/8)*8
    # if h%8!=0 or w%8!=0:
    #     image = image.resize((h_size , w_size))

    print("Original image shape : ",np.array(image).shape)
    # image.save("original_"+str(input_file.split('_')[0]))
    ...
    if rows % 8 == cols % 8 == 0:
        ...
        # print(blocks_count)
        output_file=input_file.split('_')[1][:-4]+"_"+str(rows)+"_"+str(cols)
    else:
    ...
    for i in range(0, rows, 8):
        for j in range(0, cols, 8):
            ...
            for k in range(3):
                ...
                # quant_matrix = quantize(dct_matrix,'lum' )
