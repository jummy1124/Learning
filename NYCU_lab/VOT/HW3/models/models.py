...
def create_modules(module_defs, img_size, cfg):
    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef['type'] == 'selayer':
            ...
            # modules.add_module('ada_avgpool', nn.AdaptiveAvgPool2d(1))
            # modules.add_module('linear1', nn.Linear(output_filters[-1], 8))
            # modules.add_module('activation1', nn.ReLU(inplace=True))
            # modules.add_module('linear2', nn.Linear(8, output_filters[-1]))
            # modules.add_module('activation2', nn.Sigmoid())
            # print(modules)
